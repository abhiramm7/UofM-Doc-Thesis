...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/RL_main_fig_1_final.eps}
...
\section{Methods}
\subsection{\textbf{Reinforcement learning for stormwater systems}}
...
\end{align}
%\mathbb{E} \left[r_t+ \gamma r_{t+1}+ \gamma^2 r_{t+2}+ \ldots \Big| s_t \right] =
% = \mathbb{E} \left[r_t+ \gamma r_{t+1}+ \gamma^2 r_{t+2}+ \ldots \Big| s_t,a_t \right]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/casestudy_fig_2.eps}
...
\subsection{\textbf{Scenario 1: Control of a single basin}}
...
\subsection{\textbf{Scenario 1: Control of single basin}}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/single_pond.eps}
...
\subsection{\textbf{Scenario 2: Control of multiple basins}}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/system_tarining.eps}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/system_scale_2.eps}
    \caption{RL agent controlling multiple stormwater basins during a 24-hour, 10-year storm event. Control actions at each of the controlled basins are shown as valve settings in the fourth row of the plot. In this scenario, the agent achieves a high reward, by maximizing the storage utilization in the most upstream control asset (4) and regulating the outflow from it to meet the downstream objectives. \edit{Difference in the scale of Y-axis in second row demonstrates the wide range of inflows in the network.}}
    \label{fig:6}
\end{figure}
...
Such approaches should be evaluated in the future since they may permit more domain knowledge from water resources to be embedded into \replace{the learning problem} \edit{training the controller}.

\edit{It is important to note that the Equal-filling algorithm outperforms the RL agent in this study (SI-\ref{SI:equalfill}).
